...
GIFs are automatically saved to: outputs/visualizations/

Usage Examples:
    # Single scenario visualization
    python visualize_model.py --model model.zip --scenario highway --modality lidar

    # Batch test all scenarios and modalities
    python visualize_model.py --model model.zip --batch --save-gif

    # Create custom GIF with specific parameters
    python visualize_model.py --model model.zip --scenario merge --modality lidar --max-steps 200 --save-gif --gif-fps 15 --output my_demo.gif
"""
...
def ensure_visualization_dir():
    """Ensure the outputs/visualizations directory exists."""
    viz_dir = Path("outputs/visualizations")
    viz_dir.mkdir(parents=True, exist_ok=True)
    return viz_dir


def get_gif_filename(scenario: str, modality: str, episode: int = 1, custom_name: str = None):
    """Generate a standardized GIF filename."""
    if custom_name:
        if not custom_name.endswith('.gif'):
            custom_name += '.gif'
        return custom_name

    # Generate descriptive filename
    timestamp = time.strftime("%Y%m%d_%H%M%S")
    filename = f"{scenario}_{modality}_episode_{episode}_{timestamp}.gif"
    return filename
        ...
        for episode in range(episodes):
            ...
            if save_gif and gif_frames and HAS_PIL:
                ...
                if output_filename:
                    ...
                    gif_filename = get_gif_filename(scenario, modality, episode + 1)

                # Full path to save GIF
                gif_path = viz_dir / gif_filename
...
def main():
    parser = argparse.ArgumentParser(description="Visualize trained autonomous driving model across all scenarios and modalities")
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output GIF filename (default: outputs/visualizations/SCENARIO_MODALITY_episode_X_TIMESTAMP.gif)"
    )
    parser.add_argument(
        "--batch",
        action="store_true",
        ...
        for scenario in scenarios:
            for modality in modalities:
                ...
                visualize_model(args.model, scenario, modality, 1, args.max_steps,
                              args.save_gif, args.gif_fps, args.output)
    else:
