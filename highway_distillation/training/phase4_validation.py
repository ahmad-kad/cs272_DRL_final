...
def evaluate_agent(model_path: str, env, num_episodes: int = 100,
    ...
    results['behavioral_analysis'] = compute_behavioral_analysis(results)
    ...
    if results['behavioral_analysis']:
        analysis = results['behavioral_analysis']
        logger.info(f"Behavioral Analysis: Consistency={analysis['consistency_score']:.3f}, "
                   f"Adaptability={analysis['adaptability_score']:.3f}")

    return results


def compute_behavioral_analysis(results):
    """Analyze agent behavior patterns from evaluation results."""
    ...
    analysis = {
        # Decision diversity: how varied are the action choices?
        'decision_diversity': len(set(results.get('actions_taken', []))) / max(1, len(results.get('actions_taken', []))),

        # Risk-taking behavior: frequency of high-risk actions
        'risk_taking': np.mean([1 if r < -0.5 else 0 for r in rewards]),

        # Behavioral consistency: how predictable is performance?
        'consistency_score': 1.0 - np.std(rewards) / (np.mean(rewards) + 1e-6),

        # Exploration efficiency: balance between exploration and exploitation
        'exploration_ratio': np.std(lengths) / (np.mean(lengths) + 1e-6),

        # Learning adaptability: how well agent handles different scenarios
        'adaptability_score': np.mean(successes) * (1.0 - np.std(successes)),

        # Behavioral stability: consistency across episodes
        'behavioral_stability': 1.0 / (1.0 + np.std(lengths)),
    }
